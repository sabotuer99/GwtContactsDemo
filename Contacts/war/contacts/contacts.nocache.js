...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "contacts" because\n';
  ...
  $wnd.__gwt_activeModules['contacts'] = {
    'moduleName' : 'contacts',
    'bindings' : function() {
      return {};
    }
 ...
 * "/contacts.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['contacts'].superdevmode = true;

  var expectedSuffix = '/contacts.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + contacts + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:contacts';
  var devModeSessionKey = '__gwtDevModeSession:contacts';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/contacts/contacts.nocache.js';
  ...
  $wnd.__gwt_activeModules['contacts'].canRedirect = true;
  $wnd.__gwt_activeModules['contacts'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/contacts?user.agent=' + ua + '&_callback=' + callback;
